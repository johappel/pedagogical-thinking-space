...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function MomentEditor({ moment, functions, sessionId, onClose, onSaved }) {
      ...
      const [error, setError] = React.useState(''); const [busy, setBusy] = React.useState(false); const [impact, setImpact] = React.useState(null);
      const update = (name, value) => { setForm((old) => ({ ...old, [name]: value })); setImpact(null); };
      const field = (name, label, multiline) => h('label', { className: 'pmw-field' }, h('span', null, label), multiline ? h('textarea', { value: form[name], onChange: (e) => update(name, e.target.value) }) : h('input', { value: form[name], onChange: (e) => update(name, e.target.value) }));
      const payload = () => ({ ...form, material_needs: lines(form.material_needs), open_questions: lines(form.open_questions) });
      const commit = async (fields) => { setBusy(true); setError(''); try {
        await post('/api/pts-landscape/moment', { sessionId, momentId: moment.id, fields });
        onSaved(); onClose();
      } catch (e) { setError(String(e?.message || e)); } finally { setBusy(false); } };
      const save = async () => { setBusy(true); setError(''); try {
        const fields = payload(); const result = await post('/api/pts-moment-workshop/impact', { sessionId, momentId: moment.id, fields });
        if (result.impact?.requiresReview) { setImpact(result.impact); return; }
        await commit(fields);
      } catch (e) { setError(String(e?.message || e)); } finally { setBusy(false); } };
      const confirmSave = async () => { const fields = payload(); setImpact(null); await commit(fields); };
      const impactPanel = impact ? h('section', { className: 'pmw-impact', role: 'alert' },
        h('h3', null, 'Aenderungsfolgen pruefen'),
        h('p', null, `Dieser Lernmoment wird bereits ${impact.usages.length === 1 ? 'in einer Unterrichtsphase' : `in ${impact.usages.length} Unterrichtsphasen`} verwendet.`),
        h('p', null, 'Die Unterrichtsreihe wird durch diese Aenderung nicht automatisch umgeschrieben.'),
        h('p', null, 'Geaenderte Angaben:'), h('ul', null, impact.changedFields.map((field) => h('li', { key: field.id }, field.label))),
        h('ul', null, impact.usages.map((use) => h('li', { key: `${use.lessonId}:${use.phaseId}` }, `${use.lessonTitle} -> ${use.phaseTitle}`))),
        impact.redThreadCheck ? h('p', { className: 'pmw-impact-note' }, 'Vor dem Speichern pruefen: Passt die Aenderung noch zum roten Faden und zu den betroffenen Lernmomenten, etwa "Faktencheck"?') : null,
        h('div', { className: 'pmw-impact-actions' },
          h('button', { onClick: () => setImpact(null), disabled: busy }, 'Zurueck zur Bearbeitung'),
          h('button', { onClick: () => { focus(sessionId, 'moment', moment.id, true); onClose(); }, disabled: busy }, 'Im Gespraech pruefen'),
          h('button', { onClick: confirmSave, disabled: busy }, 'Speichern und Verwendungen pruefen')),
      ) : null;
      return h('div', { className: 'pmw-overlay', role: 'dialog', 'aria-modal': 'true', 'aria-label': `Lernmoment ${moment.title || moment.id} bearbeiten` },
        h('div', { className: 'pmw-dialog' }, h('header', null, h('h2', null, 'Lernmoment-Werkstatt'), h('button', { onClick: onClose, 'aria-label': 'Schliessen' }, 'X')),
          h('p', { className: 'pmw-muted' }, moment.id), field('title', 'Titel'), field('type', 'Typ'),
          h('label', { className: 'pmw-field' }, h('span', null, 'Didaktische Funktion'), h('select', { value: form.function, onChange: (e) => update('function', e.target.value) },
          ...
          field('learning_activity', 'Lernaktivitaet', true), field('expected_experience', 'Erwartete Lernerfahrung', true), field('material_needs', 'Materialbedarfe (eine Zeile je Eintrag)', true), field('open_questions', 'Offene Fragen (eine Zeile je Frage)', true),
          h('p', { className: 'pmw-muted' }, `Reifegrad: ${moment.status || 'draft'}${moment.provenance ? ` | Herkunft: ${moment.provenance}` : ''}`), error ? h('p', { role: 'alert', className: 'pmw-error' }, error) : null, impactPanel,
